...
TRACK_STAGES = {
    ...
    # 2026-09-22: ①Bezier 가 나가면서 부모 노트 TSE_SEM.md 의 `stage:` 가 submitted 로
    # 올라갔다(노트의 설명 — 「스칼라는 선두 편(①)을 따른다」). ① 은 그 값이 맞지만
    # ② 는 아직 ① 의 접수번호를 기다리는 초고다. 물려받기를 막지 않으면 ② 가 나간 것처럼
    # 그려진다. 볼트 쪽 짝은 Dashboards/프로젝트현황.md 분리트랙의 `투고상태: "draft"` 다.
    "TSE_SEM2_Tensor": "draft",
    ...
    # 2026-09-21: TomoSh4 도 나갔다(이희란 교수 교신, 원고번호 미발급). 이제 볼트
    # paper_stage 는 둘 다 submitted 인데, 부모 노트의 `stage:` 는 아직 draft 다 —
    # 그래서 두 줄 다 여기 남는다. 부모 stage 가 submitted 로 올라가면 이 표는 비운다.
    "TSE_TomoSh4": "submitted",
}
...
HYPEREDGES = [
    ...
    # 2026-09-22: 사용자 지시로 TSE_SEM3_AutoTune 을 다시 넣는다. 처음엔 단독 저자 그대로인
    ...
    # 2026-09-21: 볼트가 Jeon_DLPOrient 를 coauthors: [전석진] 으로 **확정**했다(사용자 —
    # 출력·시험을 그쪽 연구실이 맡는다). 방대석 훌이 하루 만에 그랬듯 이 훌도 더는 후보
    # 예외가 아니다. 노드 정보의 「공저 후보」 줄은 볼트에서 coauthor_candidate 가 빠지면서
    # 재생성이 스스로 거둔다.
    {"label": "전석진 교수님",
     "kind": "coauthor",
...
QUALITY_ROWS = [
    ...
    # 2026-09-22: 나갔다. 저자가 「박명진 · 은종현* · 설인환*」로 확정되면서 09-11 의
    # 「대학원생」 자리표시자가 닫혔고, 학회 양식 변환은 박명진 학생이 했다(볼트 submission).
    # 2026-09-22 中 → 下: 투고 뒤에는 투고한 저널의 등급으로 본다(위 「투고 뒤 등급」).
    # 한국복합재료학회지는 국내지다 — cfmsCIPC 가 09-14 에 같은 규칙으로 내려간 자리다.
    ("PFTF_GFiberCT", "PFTF_GFiberCT", "하", "합성 144건·NCF 유리섬유 복합재 X선 CT 17쌍(원저자 분할) 사전등록 검증, B5/M1 대비 사례별 144/144·17/17 승, 위상 오차 0; **2026-09-22 한국복합재료학회지 투고 완료**(은종현 교수 측 투고, 저자 박명진·은종현*·설인환*). 원고번호 S-26-0084(09-22 접수 확인 메일). PFTF/local-SPD 우월성 제외"),
    ...
    # 2026-09-22: ① 이 나갔다. 「금주 투고」 예고가 실행됐고 저자·COI 는 투고본으로 닫혔다.
    ("TSE_SEM1_Bezier", "TSE_SEM1_Bezier", "하",
     "① 방향 리프팅+3차 Bézier 관 재구성; 실제 SEM IoU 0.814·팬텀 12종 배향 오차 2.3–11.6°, **2026-09-22 한국복합재료학회지 투고 완료**(은종현 교수 측 투고). 원고번호 S-26-0085(09-22 접수 확인 메일) — ②Tensor 가 이 번호로 companion 인용을 확정한다"),
    ("TSE_SEM2_Tensor", "TSE_SEM2_Tensor", "하",
     "② 이진화 없는 배향 텐서장, 하부층 포함; 저배율 SEM 9장 observed fraction 0.86–0.93, 원고 완성. ① 이 2026-09-22 에 나가 접수번호 S-26-0085 를 받았으므로, 그 번호로 companion 인용을 확정한 뒤 같은 **한국복합재료학회지**에 은종현 교수 측이 투고한다"),
    ("TSE_SEM3_AutoTune", "TSE_SEM3_AutoTune", "하",
     "③ 증거 제약 기반 자동 파라미터 선택; 게이트 C1~C4 전부 처리(2026-09-14), 사전 등록한 확인 실험은 불통과여서 부정 결과·경계 설정 논문으로 주장이 좁아졌다. 투고처 한국섬유공학회지 확정. 저자는 대학원생·은종현·설인환*(09-14 단독 → 2026-09-22 공저로 되돌림)이고 사사만 금오공대 대학 연구과제비(2026-2027)로 ①② 와 갈렸고, 투고 시점이 2027-03-01 이후로 정해져 그때까지 보류(일정 사유)"),
    ...
    # 2026-09-21: ④ 도 나갔다. 「이 원고가 그 번호를 받아 적고 투고한다」는 예고가 실행됐다 —
    # 뒤집힌 익명 companion 인용 방향은 이 투고로 닫혔다(볼트 submission).
    ("TSE_TomoSh4", "TSE_TomoSh4", "하",
     "분할 트랙 — 이진 법선 패널티를 연속 가중으로 바꿔 점-뼈대 거리 분할의 파편화를 줄인다; **2026-09-21 한국섬유공학회지 투고 완료**(이희란 교수 교신, 저자 강지언·설인환·이희란). ⏳ 원고번호는 아직 없다 — 접수 승인 메일로 발급된다. TomoSh5(26M-09-036)를 익명 companion 으로 인용해 나갔고, 뒤집혔던 인용 방향은 이로써 닫혔다"),
...
if preserve_extended_quality:
    s = _update_json_object_constant(
        ...
        "STATUS_BADGES",
        {
            ...
            # 2026-09-22: 나갔다(은종현 교수 측 투고). 원고번호는 아직 못 받았다.
            "PFTF_GFiberCT": "한국복합재료학회지 투고 완료 (2026-09-22, 은종현 교수 측 투고 · 원고번호 S-26-0084)",
            ...
            # 2026-09-22: ① 이 나갔다. ② 는 ① 의 접수번호를 기다리므로 배지가 그대로다.
            "TSE_SEM1_Bezier": "한국복합재료학회지 투고 완료 (2026-09-22, 은종현 교수 측 투고 · 원고번호 S-26-0085)",
            ...
            # 2026-09-21: ④ 도 나갔다(이희란 교수 교신). 원고번호는 접수 승인 메일이 와야
            # 생기므로 배지에는 날짜만 적고, 번호가 오면 ⑤ 와 같은 꼴로 고친다.
            "TSE_TomoSh4": "한국섬유공학회지 투고 완료 (2026-09-21, 원고번호 미발급)",
    ...
    existing_quality_js = _update_json_object_constant(
        ...
        {
            # 2026-09-22 볼트 동기화. 투고로 게재지·저자·서지가 닫히고 병목이 행정과 정정으로 옮겨 갔다.
            "PFTF_GFiberCT": "원고번호 확인과 제출본 정정이다(2026-09-22 투고) — 제출본 3.1절의 F-점수 차이 0.1978 이 동결값 0.198937 과 다르고, 제출본 DOCX 가 리포에 미커밋이며 CRediT 분담이 미기록이다. 주장 제한은 그대로(시편 1개·ρ=4 는 한 패널의 값, PFTF/local-SPD 우월성 주장 금지)",
            # 2026-09-11: 볼트 TSE_SEM.md 「투고 일정」의 트랙별 남은 일.
            # 2026-09-16: ①② 는 은종현 교수 측이 한국복합재료학회지에 투고한다(박명진 학생 학회 양식 변환).
            # 2026-09-22: ① 이 나갔다. 09-13 심사의견 미반영 4건과 저자·COI 는 투고본으로 닫혔고,
            # ② 는 「① 접수 후 투고」의 앞 절반이 닫혀 접수번호 대기만 남았다.
            "TSE_SEM1_Bezier": "원고번호 확인 대기 (2026-09-22 투고, 은종현 교수 측) — 그다음은 심사 결과. ②Tensor 가 이 접수번호로 companion 인용을 확정한다",
            "TSE_SEM2_Tensor": "① 접수번호 대기 (①은 2026-09-22 투고 완료) → 그 번호로 companion 인용 확정 후 은종현 교수 측 투고(한국복합재료학회지), ①과의 방법·그림·검증 주장 중복 정리",
            # 2026-09-14: C1~C4 가 전부 처리됐고 저자가 설인환 단독이 됐다가, 2026-09-22 에
            # 공저(대학원생·은종현·설인환*)로 되돌아와 원고 저자 블록 수정이 다시 남았다.  투고
            # 시점은 2027-03-01 이후 그대로다.
            "TSE_SEM3_AutoTune": "2027-03-01 까지 보류(일정), ② 접수번호로 상호 인용 확정, 23면 → 20면 분량 조절, 대학원생 실명·기여 분담 확정(저자 블록은 09-22 에 공저로 고쳤다)",
            # 볼트 Tomo_Shell2026.md 의 next_gate 를 트랙별로 나눈 것이다.
            # 2026-09-21: ④ 투고로 「저자 확인 4건 반영 → 투고」가 닫혔다. 남은 것은 행정이다.
            "TSE_TomoSh4": "원고번호 발급 대기 (2026-09-21 투고) — 접수 승인 메일이 오면 submission·badge 에 적는다. 그다음은 심사 결과 대기",
            ...
            # PFTF_Mold 도 같은 공저자·같은 일정이지만 그 노트에는 bottleneck 필드가 없어
            # 여기 안 적는다(next_gate 에만 있다 — 포트폴리오 표가 그것을 싣는다).
            "SFTF_InjMold": "공저자 일정이다 — 원고를 방대석 교수에게 전달했고(2026-09-21) 내년 연구년에 볼 예정이라 투고가 2년 뒤로 밀릴 수 있다. 행정 둘(교신저자 ORCID·1저자 실명)은 그쪽이 채운다. 분량·형식·인용은 닫혀 우리 쪽에 남은 일이 없다",
        },
...
SEM_TRACKS = [
    ...
    },
    {
        ...
        "scope": "③ AutoTune 트랙(보류 — 2027-03-01 이후 투고) · 저자 대학원생·은종현·설인환*(2026-09-22 공저로)",
